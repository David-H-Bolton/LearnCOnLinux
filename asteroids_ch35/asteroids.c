// Chapter 35 Linux - Added collision detection code
#include "hr_time.h"
#include <time.h>
...
// Load all masks if any return 0 then failed to load so exit 0
int LoadMasks() {
	maskErrorFile = NULL;
	if (!LoadMask("masks/bullet.msk", 3, 1, &bulletmask[0][0][0])) return 0; // bullet
...
// clears the list of cells each frame
void ClearCellList() {
...
void InitSetup() {
	...
	LoadTextures();
	int success=Mix_OpenAudio(44000, AUDIO_F32SYS, 2, 8192);
	if (success==-1 ) {
...
void PlayASound(int soundindex) {
	...
	// commented out because multiple explosions use all available channels
/* 	if (success == -1) {
		LogError2("IUnable to play because %s",Mix_GetError());
	} */

}

// Used to put an image on screen- used for backdrop
...
void DrawPlayerShip() {
	...
	if (!debugFlag) return;
	// code after here is only run if debugFlag is non-zero 
	target.w = 64;
	target.h = 64;
...
void DrawEveryThing() {	
	startTimer(&s);
	renderTexture(textures[PLBACKDROP], 0, 0); // Put starield image on first
	DrawAsteroids();
	DrawPlayerShip();
	DrawBullets();
	DrawExplosions();
...
// called to toggle buffers and reset framecount
void RenderEverything() {
	SDL_RenderPresent(renderer); // make offscreen buffer visible
	frameCount++;
...
void CheckJump() {
	...
	if (jumpFlag && SDL_GetTicks() - jumpTimer > 30) { 
...
int Overlap(pfirstpart object1, pfirstpart object2, SDL_Rect * rect,int * bangx,int * bangy) {
	...
	int y2 = object2->r.y;
	int x2 = object2->r.x;
	int dir1 = object1->type == 1 ? 0: object1->rotdir;
	int dir2 = object2->type == 1 ? 0 : object2->rotdir;
	...
	{	
		...
		if (!paused) {
			...
			*/
			...
			CycleExplosions();
			DoCoolDown();
			ShowCells();  // Comment this out if you don't want to see the grid
		}
		RenderEverything();
