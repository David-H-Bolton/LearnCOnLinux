...
void UpdateCaption() {
	snprintf(buffer2, sizeof(buffer2), "%10.6f #a=%d", diff(&s),numAsteroids);
...
void DrawAsteroids() {
	...
	for (int i = 0; i<MAXASTEROIDS; i++) {
		if (asteroids[i].active==1) {
			...
			int size = asteroids[i].size; // 0-3
			int asize = sizes[size];      // asteroid size 280,140, 70,35
			target.h = asize;
			target.w = asize;
			...
			TextAt(target.x + 25, target.y + 55, SDL_ltoa(size, buff, 10));
		}
	}
}
