...
const char *texturenames[NUMTEXTURES] = {"images/starfield.png", "images/text.png", "images/playership.png", "images/debug.png",
										 "images/a1.png", "images/a2.png", "images/a3.png", "images/a4.png", "images/bullet.png", "images/smallship.png"};
...
void UpdateCaption()
{
	...
	snprintf(buffer2, sizeof(buffer2), "%10.6f Pi=%d temp=%5.2f C", diff(&s),piFlag,temp);
...
void DrawAsteroids()
{
	...
	for (int i = 0; i < MAXASTEROIDS; i++)
	{
		...
		{
			numAsteroids++;
			int sizeIndex = asteroids[i].size;
...
void AddAsteroid(int size)
{
	...
	if (index == -1) // table full so quietly exit
		return;
	if (size == -1)
	{ // Use -1 to sepcify a random size
		size = Random(4) - 1;
	}
...
void doFireBullet()
{
	...
	if (index == -1)
		return; // no free slots as all bullets in play
