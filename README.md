# LearnCOnLinux
The initial files have been updated including Raspberry PO compatability.
These are the files for the ebook Learn C Games Programming for beginners on Linux which is now available on [https://amzn.eu/d/87tHW3A](url)Amazon (UK). All graphics and sounds files are included.

Because Git forbids hidden files, I've renamed .vscode folders to vscode. These folders need to be renamed back after extracting. 
